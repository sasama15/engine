...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* ParticleManager3d::device = nullptr;
Camera* ParticleManager3d::camera = nullptr;
ComPtr<ID3D12RootSignature> ParticleManager3d::rootsignature;
ComPtr<ID3D12PipelineState> ParticleManager3d::pipelinestate;

void ParticleManager3d::Initialize()
{
	HRESULT result;
	// 定数バッファの生成
...
void ParticleManager3d::CreateGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources/shaders/ParticleVS.hlsl",					// シェーダファイル
		nullptr,
...
void ParticleManager3d::Update()
{
	...
	if (SUCCEEDED(result)) {
		...
		constMap->viewProjectBillboard = matViewProjection;
...
void ParticleManager3d::Draw(ID3D12GraphicsCommandList* cmdList)
{
	// モデルの割り当てがなければ描画しない
	if (fbxModel == nullptr) {
		return;
...
void ParticleManager3d::PlayAnimation(const int& AnimationNum, const bool& isLoop)
{
