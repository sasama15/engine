...
void FbxLoader::ParseMeshFaces(FbxModel* fbxModel, FbxMesh* fbxMesh)
{
    ...
    for (int i = 0; i < polygonCount; i++) {
        ...
        const int polygonSize = fbxMesh->GetPolygonSize(1);
...
void FbxLoader::LoadTexture(FbxModel* fbxModel, const std::string& fullpath)
{
    ...
    // WICテクスチャのロード
    TexMetadata& matadata = fbxModel->metadata;
...
void FbxLoader::ParseSkin(FbxModel* fbxModel, FbxMesh* fbxMesh)
{
    ...
    if (fbxSkin == nullptr) {
        ...
        for (int i = 0; i < fbxModel->vertices.size(); i++) {
            ...
            fbxModel->vertices[i].boneWheight[0] = 1.0f;
        }
        return;
    }
    // ボーン配列の参照
    ...
    for (int i = 0; i < vertices.size(); i++) {
        ...
        for (auto& weightSet : weightList) {
            ...
            vertices[i].boneWheight[weightArrayIndex] = weightSet.weight;
            // 4つに達したら終了
            if (++weightArrayIndex >= FbxModel::MAX_BONE_INDICES) {
                float weight = 0.0f;
                // 2番目以降のウェイトを合計
                for (int j = 1; j < FbxModel::MAX_BONE_INDICES; j++) {
                    weight += vertices[i].boneWheight[j];
                }
                // 合計で1.0f(100%)になるように調整
                vertices[i].boneWheight[0] = 1.0f - weight;
                break;
            }
        }
    }
