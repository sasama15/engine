...
const float MultiTexture::clearColor[4] = { 0.25f, 0.5f, 0.1f, 0.0f };	// 緑っぽい色
...
void MultiTexture::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	for (int i = 0; i < 2; i++) {
...
void MultiTexture::Draw(ID3D12GraphicsCommandList* cmdList)
{
...
void MultiTexture::PostDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	for (int i = 0; i < 2; i++) {
...
void MultiTexture::CreateGraphicsPipelineState()
{
	...
	(
		L"Resources/shaders/MultiTextureVS.hlsl",			// シェーダファイル名
		nullptr,
	...
	result = D3DCompileFromFile
	(
		L"Resources/shaders/MultiTexturePS.hlsl",					// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,					// インクルード可能にする
